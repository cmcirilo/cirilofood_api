...
public class SpringFoxConfig implements WebMvcConfigurer {
    ...
    private SecurityContext securityContext() {
        var securityReferente = SecurityReference.builder()
                .reference("CiriloFood")
                .scopes(scopes().toArray(new AuthorizationScope[0]))
                .build();

        return SecurityContext.builder()
                .securityReferences(Arrays.asList(securityReferente))
                .forPaths(PathSelectors.any())
                .build();
    }

    private List<GrantType> grantTypes() {
        return Arrays.asList(new ResourceOwnerPasswordCredentialsGrant("http://api.cirilofood.local:8080/oauth/token"));
    }
